...
class Structure:
    ...
    def __init__(self, num_nodes: int,
        ...
        self.dof_per_elem = self.dof_per_elem.at[:, :6].set(6 * self.connectivity[:, [0]] + jnp.arange(6)[None, :])
        self.dof_per_elem = self.dof_per_elem.at[:, 6:].set(6 * self.connectivity[:, [1]] + jnp.arange(6)[None, :])
    ...
    def set_design_variables(self, coords: Array, k_cs: Array, m_cs: Optional[Array]) -> None:
        # populate arrays
        ...
        dy = jnp.cross(dx_unit, self.plane_vector, axis=-1)   # vector in plane[n_elem, 3]
        dy_unit = dy / jnp.linalg.norm(dy, axis=-1)[:, None]   # [n_elem, 3]
        dz_unit = jnp.cross(dx_unit, dy_unit, axis=-1)   # unit vector out of plane, [n_elem, 3]
        ...
        self.k = self.k.at[...].set(jnp.einsum('ijk,ikl,iml->ijm', chi0, self.k_cs, chi0))
        self.m = self.m.at[...].set(jnp.einsum("ijk,ikl,iml->ijm", chi0, self.m_cs, chi0))
    ...
    def make_k_t(self, d: Array, include_geometric: bool = True) -> Array:
        ...
        k_t_entries = jnp.einsum("ikj,ikl,ilm->ijm", pd, self.k, pd_l)  # [n_elem, 12, 12]
