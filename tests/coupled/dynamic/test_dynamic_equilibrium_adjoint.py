...
class TestDynamicEquilibriumAdjoint:
    def test_dynamic_equilibrium_adjoint(self, plot: bool = False):
        ...
        wing = make_cantilever_wing(m=m, m_star=m_star, c_ref=c_ref, b_ref=b_ref, k_cs=k_cs, ea=0.25, n_nodes=n + 1,
                                    u_inf=u_inf)
        ...
        assert jnp.allclose(dynamic_grad.aero.flowfield['u_inf'],
                            static_grad.aero.flowfield['u_inf']), "Mismatch in u_inf gradient"

        assert jnp.allclose(dynamic_grad.aero.flowfield['rho'],
