...
from aegrad.coupled.data_structures import AeroelasticFullStates, AeroelasticMinimalStates, AeroelasticDesignVariables
    ...
    def compute_p_q0_p_x(self, case: StaticAeroelastic, p_varphi_p_x: Array, solve_dofs: Array, free_wake: bool = False,
        ...
        indirect_term = AeroelasticDesignVariables(
            structure_dv=StructuralDesignVariables(**{k: indirect_dict[k] for k in dv.structure.get_vars()},
                                                   f_shape=()),
            aero_dv=AeroDesignVariables(**{k: indirect_dict[k] for k in dv.aero.get_vars()}, f_shape=()),
        )
        ...
        dv_grad_init = AeroelasticDesignVariables(
            ...
            aero_dv=AeroDesignVariables(
                ...
                flowfield={k: jnp.zeros((*j_shape, *v.shape)) for k, v in
                           self.aero.flowfield.to_design_variables().items()}, f_shape=j_shape))
