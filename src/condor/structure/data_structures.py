...
from condor.algebra.base import chi
from condor.plotting.beam import plot_beam_to_vtk
from condor.plotting.pvd import write_pvd
from condor.structure.gradients.data_structures import StructureFullStates
from condor.structure.utils import (
    get_solve_dofs,
    input_dof_index_to_tuple,
    transform_nodal_vect,
)
from condor.utils.print_utils import warn
from condor.utils.utils import index_to_arr, make_pytree
...
class StaticStructure:
    ...
    def __init__(
        ...
        self.f_ext_follower: Array | None = f_ext_follower  # (n_nodes, 6)
        self.f_ext_dead: Array | None = f_ext_dead  # (n_nodes, 6)
        self.f_ext_aero: Array | None = f_ext_aero  # (n_nodes, 6)
        self.f_grav: Array | None = f_grav  # (n_nodes, 6)
...
class DynamicStructureSnapshot:
    ...
    def plot(self, directory: os.PathLike | str, n_interp: int = 0) -> Path:
        ...
        m_iner = data.f_iner[:, 3:]  # (n_nodes, 3)
        ...
        v_dot_ang = data.v_dot[:, 3:]  # (n_nodes, 3)
        ...
        # element force data
        f_elem_lin = data.f_elem[:, :3]  # (n_elem, 3)
        f_elem_ang = data.f_elem[:, 3:]  # (n_elem, 3)
...
class DynamicStructure:
    ...
    def __init__(
        ...
        self.f_ext_follower: Array | None = f_ext_follower  # (n_tstep, n_nodes, 6)
        self.f_ext_dead: Array | None = f_ext_dead  # (n_tstep, n_nodes, 6)
        self.f_ext_aero: Array | None = f_ext_aero  # (n_tstep, n_nodes, 6)
        self.f_grav: Array | None = f_grav  # (n_tstep, n_nodes, 6)
    ...
    def __getitem__(self, i_ts: int) -> DynamicStructureSnapshot:
        """Extract dynamic structure initial snapshot at a specific time index."""
    ...
    def _transform(self, rmat: Array) -> None:
        r"""
        Transform orientation-dependent results between frames using the nodal chi transformation matrices.
        :param rmat: Nodal rotations, (n_tstep, n_nodes, 3, 3)
        """
