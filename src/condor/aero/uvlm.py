...
from condor.aero.aic import compute_aic_solve, compute_v_ind
from condor.aero.data_structures import (
    AeroSnapshot,
    DynamicAeroCase,
    GridDiscretisation,
)
from condor.aero.flowfields import FlowField
...
from condor.algebra.base import ADMode, construct_approximation, jacrev_custom
from condor.algebra.se3 import vect_product as se3_vect_product
from condor.algebra.test_routines import check_if_all_se3_a, check_if_all_se3_g
from condor.coupled.data_structures import AeroelasticDesignVariables
from condor.structure import BeamStructure, DynamicStructure
from condor.structure.utils import transform_nodal_vect
from condor.utils.constants import HORSESHOE_LENGTH
from condor.utils.utils import make_pytree

if TYPE_CHECKING:
    from condor.aero.linear.linear_uvlm import LinearUVLM, LinearWakeType
...
class UVLM:
    ...
    def __init__(
        ...
        May be passed as a sequence of either the GridDiscretisation class or a tuple of integers ordered as (m, n,
        m_star).
        ...
        for grid in grid_shapes:
            if isinstance(grid, Sequence):
                ...
                raise TypeError(
                    "Grid arr_list_shapes must be either a Sequence of three integers or a GridDiscretisation instance"
                )
        ...
        self._hg0: Array | None = None
        self._zeta_b0: ArrayList | None = None
        self._zeta_b_ref: ArrayList | None = None
        self._zeta_w_ref: ArrayList | None = None
        ...
        self._flowfield: FlowField | None = None
        ...
        self._delta_w: list[Array | None] | None = None
    ...
    def zeta_b0(self) -> ArrayList:
        ...
        if self._zeta_b0 is None:
            raise ValueError("Design variable zeta_b0 has not been set.")
    ...
    def set_design_variables(
        ...
        self._zeta_b0 = x0_aero_arraylist
        ...
        self._hg0: Array = hg0
        self._zeta_b_ref = self.hg_to_zeta_b(hg_n=hg0, cs_ang_n=self.cs_ang0)
        ...
        self._zeta_w_ref = self.initialise_wake()
    ...
    def case_from_dv(self, dv: AeroDesignVariables) -> UVLM:
        ...
        inner_case.set_design_variables(
            ...
            zeta_b0=dv.zeta_b0 if dv.zeta_b0 is not None else self.zeta_b0,
            hg0=self.hg_ref,
            reference_cs_angles={
